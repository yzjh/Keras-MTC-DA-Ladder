...
y_l_t = base_model(Inp_l)
y_u_t = base_model(Inp_u)
y_l_t = Flatten()(y_l_t)
y_u_t = Flatten()(y_u_t)
y_l = new_base_model(Inp_l)
y_u = new_base_model(Inp_u)
y_l = Flatten()(y_l)
y_u = Flatten()(y_u)

# mmd距离
mmd_l = mk_mmd_loss(y_l_t, y_l)
mmd_u = mk_mmd_loss(y_u_t, y_u)
# 欧氏距离
# mmd_l = tf.reduce_mean(Lambda(lambda a: (a[0]-a[1])**2)((y_l_t, y_l)))
# mmd_u = tf.reduce_mean(Lambda(lambda a: (a[0]-a[1])**2)((y_u_t, y_u)))
# cos距离
# mmd_l = tf.reduce_mean(Lambda(lambda a: ((a[0]**2)*(a[1]**2)-a[0]*a[1])/(a[0]**2)*(a[1]**2))((y_l_t, y_l)))
# mmd_u = tf.reduce_mean(Lambda(lambda a: ((a[0]**2)*(a[1]**2)-a[0]*a[1])/(a[0]**2)*(a[1]**2))((y_u_t, y_u)))
# KL散度
# mmd_l = keras.losses.KLDivergence()(y_l_t, y_l)
# mmd_u = keras.losses.KLDivergence()(y_u_t, y_u)
...
model_trans.add_loss(lmd * mmd_l)
model_trans.add_loss(lmd * mmd_u)
#model_trans.add_metric(u_cost, name="den_loss")
#model_trans.add_metric(mmd_l, name="mmd_l")
#model_trans.add_metric(mmd_u, name="mmd_u")
...
data_tar_train, label_tar_train, data_tar_val, label_tar_val, data_tar_test, label_tar_test,\
    data_tar_train_u, data_tar_val_u = prepare_data('/trans', val_rate=0.2, trans_rate=0.15, is_semi=True)
...
model_trans.compile(loss="categorical_crossentropy",
              optimizer=keras.optimizers.Adam(lr=0.02),#keras.optimizers.Adam(lr=0.02),
              metrics=["accuracy"])
...
# second stage
history_2 = model_trans.fit([data_tar_train, data_tar_train_u],
                            label_tar_train,
                            batch_size=batch_size,
                            epochs=epochs_trans,
